...
type Item = { id: CheckId; label: string; required?: boolean; weight: number };
...
type Form = {
  ...
  planRR: string;
  notes: string;
  chartsComments: string;
};

type CheckState = Record<CheckId, boolean>;

// ---- Data ------------------------------------------------------------------
const GROUPS: Group[] = [
  {
    id: "context",
    title: "Контекст (HTF/MTF)",
    items: [
...
export default function TradeEntryChecklist() {
  const [form, setForm] = useLocalStorage<Form>("tec_form_v1", {
    date: today(),
    pair: "",
    direction: "Long",
    session: "London",
  ...
  const blankChecks = CHECK_IDS.reduce<CheckState>((acc, id) => {
    acc[id] = false;
    return acc;
  }, {} as CheckState);

  const [checks, setChecks] = useLocalStorage<CheckState>("tec_checks_v1", blankChecks);

  // Автологика
  useEffect(() => {
    setChecks((prev) => ({ ...prev, no_red_news: form.news !== "High" }));
  }, [form.news, setChecks]);

  useEffect(() => {
    const rr = parseFloat(String(form.planRR).replace(",", "."));
    if (!Number.isNaN(rr)) {
      setChecks((prev) => ({ ...prev, rr_2_plus: rr >= 2 }));
    }
  }, [form.planRR, setChecks]);

  const allItems = useMemo(() => GROUPS.flatMap((g) => g.items), []);
  const totalWeight = useMemo(
    () => allItems.reduce((s, i) => s + i.weight, 0),
    [allItems]
  );

  const score = useMemo(() => {
    const earned = allItems.reduce((s, i) => s + (checks[i.id] ? i.weight : 0), 0);
    return Math.round((earned / totalWeight) * 100);
  }, [checks, allItems, totalWeight]);

  const missing = useMemo(
    () => [...REQUIRED].filter((id) => !checks[id]),
    [checks]
  );
  ...
  const h = <K extends keyof Form>(k: K, v: Form[K]) =>
    setForm((p) => ({ ...p, [k]: v }));
  const t = (id: CheckId) => setChecks((p) => ({ ...p, [id]: !p[id] }));

  const copySummary = async () => {
    const issues = GROUPS.flatMap((g) =>
      g.items.filter((i) => !checks[i.id]).map((i) => i.label)
    );
    const txt = `# Резюме
${form.pair || "(пара?)"}, ${form.direction}, сетап ${form.setup}. Итог: ${goNoGo} (оценка ${score}/100, класс ${klass}).

# Контекст
— Сессия: ${form.session}; Новости: ${form.news}.
— Charts: ${form.chartsComments || "—"}

# План
— План RR: ${form.planRR} | Риск: ${form.riskUSD ? "$" + form.riskUSD : "(не указан)"}
          ...
          <div className="flex items-center gap-3">
            <div
              className={`px-3 py-2 rounded-2xl text-sm font-semibold ${
                goNoGo === "GO" ? "bg-emerald-600/20 text-emerald-300" : "bg-rose-600/20 text-rose-300"
              }`}
            >
              {goNoGo}
            </div>
            <div className="px-3 py-2 rounded-2xl bg-zinc-900 text-sm">
              Счёт: <b>{score}/100</b> <span className="text-zinc-400">(класс {klass})</span>
            </div>
          </div>
        </header>
          ...
          <section className="col-span-2 bg-zinc-900 rounded-2xl p-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <label className="text-xs text-zinc-400">
                ...
                <input
                  ...
                  className="w-full bg-zinc-800 rounded-xl px-3 py-2"
                />
              </label>
              <label className="text-xs text-zinc-400">
                Пара / Инструмент
                <input
                  value={form.pair}
                  onChange={(e) => h("pair", e.target.value)}
                  placeholder="EUR/USD, US100…"
                  className="w-full bg-zinc-800 rounded-xl px-3 py-2"
                />
              </label>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <label className="text-xs text-zinc-400">
                ...
                <select
                  ...
                  onChange={(e) => h("direction", e.target.value as Form["direction"])}
                  className="w-full bg-zinc-800 rounded-xl px-3 py-2"
                >
                  <option>Long</option>
                  <option>Short</option>
              ...
              <label className="text-xs text-zinc-400">
                ...
                <select
                  value={form.setup}
                  onChange={(e) => h("setup", e.target.value as Form["setup"])}
                  className="w-full bg-zinc-800 rounded-xl px-3 py-2"
                >
                  <option>OB</option>
                  <option>FVG</option>
            ...
            <div className="grid grid-cols-3 gap-3">
              <label className="text-xs text-zinc-400">
                ...
                <select
                  value={form.session}
                  onChange={(e) => h("session", e.target.value as Form["session"])}
                  className="w-full bg-zinc-800 rounded-xl px-3 py-2"
                >
              ...
              <label className="text-xs text-zinc-400">
                ...
                <select
                  value={form.news}
                  onChange={(e) => h("news", e.target.value as Form["news"])}
                  className="w-full bg-zinc-800 rounded-xl px-3 py-2"
                >
                  <option>None</option>
              ...
              <label className="text-xs text-zinc-400">
                ...
                <input
                  value={form.riskUSD}
                  onChange={(e) => h("riskUSD", e.target.value.replace(/[^0-9.,]/g, ""))}
            ...
            <div className="grid grid-cols-3 gap-3">
              <label className="text-xs text-zinc-400">
                ...
                <input
                  value={form.planRR}
                  onChange={(e) => h("planRR", e.target.value.replace(/[^0-9.,]/g, ""))}
                  placeholder="2.0"
                  className="w-full bg-zinc-800 rounded-xl px-3 py-2"
                />
              </label>
            ...
            <label className="text-xs text-zinc-400">
              Charts Comments
              <textarea
                value={form.chartsComments}
                onChange={(e) => h("chartsComments", e.target.value)}
                rows={3}
                className="w-full bg-zinc-800 rounded-xl px-3 py-2"
              />
            </label>
              ...
              <div key={g.id} className="border border-zinc-800 rounded-2xl">
                <div className="px-4 py-2 border-b border-zinc-800 flex items-center justify-between">
                  ...
                  <span className="text-xs text-zinc-400">
                    {g.items.filter((i) => checks[i.id]).length}/{g.items.length}
                  </span>
                </div>
                <div className="p-2">
                  {g.items.map((i) => (
                    <label
                      key={i.id}
                      className="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-zinc-800/50 cursor-pointer"
                    >
                      <input
                        type="checkbox"
                        className="h-4 w-4"
                      ...
                      <span className="text-sm">
                        {i.label}
                        {i.required && <span className="ml-2 text-rose-400 text-xs">(обязательно)</span>}
                      </span>
                    </label>
                  ))}
                </div>
